    ...
    for leg in legs
        do (leg) ->
            ...
            if true
                ...
                console.info(stop)
                label = "<span style='font-size: 24px;'><img src='static/images/#{google_icons[leg.routeType ? leg.mode]}' style='vertical-align: sub; height: 24px'/><span>#{leg.route} #{ if leg.mode == "BUS" then "<a  data-rel='page' stop-id=#{stop.stopCode} href='#stop-query' onclick='set_selected_stop(#{stop.stopCode})'>Stop #{  stop.stopCode 
 }</a>" else ""  } </span></span>"
